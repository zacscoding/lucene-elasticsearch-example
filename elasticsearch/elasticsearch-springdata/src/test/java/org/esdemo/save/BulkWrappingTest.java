...
public class BulkWrappingTest extends AbstractTestRunner {
    ...
    public void requestBulkAndHandleRespectively() throws Exception {
        ...
        try {
            BulkItemResponseConsumer bulkItemResponseConsumer = (res, inst) -> {
                if (res.isFailed()) {
                    SimpleLogger.printTitle("Handle Fail Response");
                    SimpleLogger.println("status : {}, instance : {}", res.status().getStatus(), inst);
                } else {
            ...
            bulkService.add(getIndexRequestWrapper(e1, bulkItemResponseConsumer));
            bulkService.add(getIndexRequestWrapper(e2, bulkItemResponseConsumer));
            bulkService.add(getIndexRequestWrapper(e3, bulkItemResponseConsumer));
    ...
    private IndexRequestWrapper getIndexRequestWrapper(BulkWrappingTestEntity e, BulkItemResponseConsumer bulkItemResponseConsumer) throws Exception {
        ...
        r.setResponseConsumer(bulkItemResponseConsumer);
        return r;
    }
}
